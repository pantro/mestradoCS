...
*/
public class JogadorAgressivo extends Jogador {
	...
	private static double alfa = 1.0; // Chance de usar cartas lacaio/magia disponíveis no turno; Do contrário não usa nada (usa apenas poder heróico).
	private static double beta = 1.0; // Chance de atacar face; Do contrário ataca lacaios aleatoriamente.
	...
	/**
	  ...
	  */
	public JogadorAgressivo(ArrayList<Carta> maoInicial, boolean primeiro){
			...
			for(int i = 0; i < mao.size(); i++){
				...
				if(card instanceof CartaLacaio && card.getMana() <= minhaMana && numMeusLacaiosLimite < 7){
					...
					if( ((CartaMagia) card).getMagiaTipo() != TipoMagia.BUFF ){ // Essa classe nao usa magias de buff
						...
						Jogada dropMagia;
						alvoRand = lacaiosOponente.size(); // ESTRATEGIA FACE (AGRESSIVO)
